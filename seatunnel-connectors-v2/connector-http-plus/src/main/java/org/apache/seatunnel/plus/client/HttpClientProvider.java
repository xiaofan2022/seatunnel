package org.apache.seatunnel.plus.client; /*
...
public class HttpClientProvider implements AutoCloseable {
    ...
    private final Retryer<Pair<Boolean, CloseableHttpResponse>> retryer;

    public HttpClientProvider(HttpParameter httpParameter, VerifyInfo verifyInfo) {
        this.httpClient = HttpClients.createDefault();
        this.retryer = buildRetryer(httpParameter);
        this.verifyInfo = verifyInfo;
    }

    private Retryer<Pair<Boolean, CloseableHttpResponse>> buildRetryer(
            HttpParameter httpParameter) {
        if (httpParameter.getRetry() < 1) {
            return RetryerBuilder.<Pair<Boolean, CloseableHttpResponse>>newBuilder().build();
        }
        return RetryerBuilder.<Pair<Boolean, CloseableHttpResponse>>newBuilder()
    ...
    private HttpResponse getResponse(HttpRequestBase request) throws Exception {
        ...
        try {
            Pair<Boolean, CloseableHttpResponse> pair = retryWithException(request);
            ...
            log.error("getResponse error message:{}", e);
        } finally {
            if (response != null) {
                try {
                    response.close();
                } catch (IOException e) {
                    log.error("close message:{}", e);
                }
            }
        }
    ...
    private Pair<Boolean, CloseableHttpResponse> retryWithException(HttpRequestBase request)
            throws Exception {
        return retryer.call(
                new Callable<Pair<Boolean, CloseableHttpResponse>>() {
                    Boolean flag = true;

                    @Override
                    public Pair<Boolean, CloseableHttpResponse> call() throws Exception {
                        CloseableHttpResponse response = httpClient.execute(request);
                        HttpEntity entity = response.getEntity();
                        String body = EntityUtils.toString(entity);
                        if (verifyInfo != null
                                && StringUtils.isNotEmpty(verifyInfo.getJsonVerifyExpression())) {
                            String expressionValue =
                                    JsonPath.read(body, verifyInfo.getJsonVerifyExpression());
                            // if(verifyInfo.getString("json_verify_expression")).toString())
                            flag = verifyInfo.getJsonVerifyValue().equals(verifyInfo);
                        }
